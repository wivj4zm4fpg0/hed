...
function [] = PostprocessHED(hed_mat_dir, edge_dir, image_width, image_height, threshold, small_edge)
...
for n = 1 : nFiles
    ...
    if ~exist(edge_path, 'file')
        E = GetEdge(filePath);
        %E = imresize(E,[image_width, image_height]);
        E_simple = SimpleEdge(E, threshold, small_edge);
        E_simple = uint8(E_simple*255);
        disp(edge_path)
        imwrite(E_simple, sprintf('%s', edge_path), 'Quality', 100);
    end
end
end
